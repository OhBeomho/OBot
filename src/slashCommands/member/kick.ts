...
export default {
  data: new SlashCommandBuilder()
    ...
    .addUserOption((option) =>
      option.setName("user").setDescription("추방할 사용자").setRequired(true)
    )
  ...
  async execute(interaction: CommandInteraction) {
    ...
    const reason = String(
      interaction.options.get("reason")?.value || member.user.tag + "이(가) 당신을 추방하였습니다."
    )
    ...
    else if (interaction.client.user.tag === kickMember.user.tag)
